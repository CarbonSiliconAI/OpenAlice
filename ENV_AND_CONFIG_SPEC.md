...
| `SIGNAL_PIPELINE_PATH` | No | `~/Projects/signal-pipeline/signals` | Signal source. Scheme-sensitive: values starting with `http://` or `https://` activate HTTP mode (GET `<base>/<date>.json`, 10 s timeout, 404 → `not_found`, other non-2xx → `network_error`). Any other value is treated as a local filesystem directory path (leading `~/` expanded to home). Both modes serve the `readSignal` MCP tool and `GET /api/trading/signal/:date?`. | `src/tool/signal.ts` (`fetchRawSignal` helper + factory), `src/main.ts` (env → factory), `src/connectors/web/routes/trading.ts` (HTTP route reuses `fetchRawSignal`) |

...
// IBKR's unset sentinels, callers can detect the sentinel shape and treat it
// as "no value" instead of rendering "$1.7e38" / "$2147483647".
//   UNSET_DECIMAL = 2^127-1, UNSET_DOUBLE = Number.MAX_VALUE, UNSET_INTEGER = INT32_MAX
...
const UNSET_INTEGER = 2147483647

export function isUnsetDecimal(v: number | string | undefined | null): boolean {
  if (v == null || v === '') return false
  if (v === UNSET_DECIMAL_STR || v === UNSET_DOUBLE_STR) return true
  const n = typeof v === 'number' ? v : Number(v)
  // Catch numeric forms of all three sentinels (in case backend ships them un-stringified).
  return n === Number(UNSET_DECIMAL_STR) || n > 1e30 || n === UNSET_INTEGER
}

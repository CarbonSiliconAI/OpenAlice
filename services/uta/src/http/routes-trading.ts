...
export function createTradingRoutes(ctx: UTAEngineContext) {
  ...
  // Remove a single staged op by zero-based index. Refuses while a pending
  // commit exists (TradingGit.drop enforces this and throws).
  app.post('/uta/:id/wallet/drop-staged', async (c) => {
    const uta = ctx.utaManager.get(c.req.param('id'))
    if (!uta) return c.json({ error: 'Account not found' }, 404)
    try {
      const body = await c.req.json().catch(() => ({}))
      ...
      const result = uta.dropStaged(body.index)
      return c.json(result)
    } catch (err) {
      return c.json({ error: err instanceof Error ? err.message : String(err) }, 400)
    }
